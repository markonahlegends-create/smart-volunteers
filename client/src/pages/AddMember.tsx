...
export default function AddMember() {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     if (unitId) {
       fetch(`/api/units/${memberType}/${unitId}`, {
         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
       })
         .then(res => res.ok ? res.json() : null)
         .then(unit => {
           if (unit) {
             const updates: any = { nama_unit: unit.nama_unit || '', jenis: unit.jenis || memberType.toUpperCase() };
             if (memberType === 'pmr' && unit.tingkat) {
               updates.tingkat = unit.tingkat;
             }
  ...
  const apiByIdMap: any = {
    pmr: membersApi.getPMRById,
    ksr: membersApi.getKSRById,
    tsr: membersApi.getTSRById,
    dds: membersApi.getDDSById,
  };
  const getById = apiByIdMap[memberType];

  useEffect(() => {
    if (isEdit && id && getById) {
      getById(Number(id)).then((member: any) => {
        setForm({
          ...
          jenis: member.jenis || memberType.toUpperCase(),
  ...
  const handleProvinsiChange = (provinsi: string) => {
    setForm(prev => ({ ...prev, provinsi, kabupaten: '', ktp_provinsi: provinsi, ktp_kabupaten: '', ktp_kecamatan: '', ktp_desa: '' }));
  };

  const handleKabupatenChange = (kabupaten: string) => {
    setForm(prev => ({ ...prev, kabupaten, ktp_kabupaten: kabupaten, ktp_kecamatan: '', ktp_desa: '' }));
  };
  ...
  const apiMap: any = {
    create: {
      pmr: membersApi.createPMR,
      ksr: membersApi.createKSR,
      tsr: membersApi.createTSR,
      dds: membersApi.createDDS,
    },
    update: {
      pmr: membersApi.updatePMR,
      ksr: membersApi.updateKSR,
      tsr: membersApi.updateTSR,
      dds: membersApi.updateDDS,
    },
  };

  const createFn = apiMap.create[memberType] || membersApi.createKSR;
  const updateFn = apiMap.update[memberType] || membersApi.updateKSR;
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['members', memberType] });
      navigate(-1);
    },
    onError: (error: any) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitError(null);
    if (!captchaVerified) {
      setSubmitError('Harap verifikasi captcha terlebih dahulu');
      return;
    }
    const requiredFields = [
      ...
      { key: 'no_nik', label: 'No Induk Kependudukan' },
      { key: 'nama', label: 'Nama Lengkap' },
      { key: 'kelamin', label: 'Jenis Kelamin' },
      { key: 'alamat_ktp', label: 'Alamat' },
      { key: 'provinsi', label: 'Provinsi' },
      { key: 'kabupaten', label: 'Kabupaten/Kota' },
      { key: 'ktp_kecamatan', label: 'Kecamatan' },
      { key: 'ktp_desa', label: 'Desa/Kelurahan' },
    ];
    ...
    console.log('Submitting:', memberType, data);
    if (isEdit && id) {
  ...
  return (
    <div>
      <Header title={isEdit ? `Edit Anggota ${memberType.toUpperCase()}` : `Tambah Anggota ${memberType.toUpperCase()}`} />
      <div className="max-w-5xl mx-auto">
        ...
        {submitError && (
          <div className="mb-4 bg-red-50 border border-red-200 text-red-700 rounded-xl p-4 flex items-start justify-between">
            <p className="text-sm font-medium whitespace-pre-line">{submitError}</p>
            <button onClick={() => setSubmitError(null)} className="ml-4 text-red-400 hover:text-red-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" name="kode_anggota" value={form.kode_anggota} onChange={(e) => setForm({ ...form, kode_anggota: e.target.value })} className={inputClass} placeholder="Kode anggota" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.provinsi} onChange={(e) => handleProvinsiChange(e.target.value)} className={selectClass}>
              ...
              <div>
                ...
                <select value={form.kabupaten} onChange={(e) => handleKabupatenChange(e.target.value)} className={selectClass}>
                  <option value="">Pilih Kabupaten/Kota</option>
                  {(KABUPATEN_OPTIONS[form.provinsi] || []).map(k => <option key={k} value={k}>{k}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                 <label className={labelClass}>No Induk Kependudukan{requiredIndicator}</label>
              ...
              <div>
                ...
                <select name="kelamin" value={form.kelamin} onChange={(e) => setForm({ ...form, kelamin: e.target.value })} className={selectClass}>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="sm:col-span-2 lg:col-span-3">
                ...
                <textarea name="alamat_ktp" value={form.alamat_ktp} onChange={(e) => setForm({ ...form, alamat_ktp: e.target.value })} className={inputClass} rows={2} placeholder="Alamat sesuai KTP" />
              </div>
               <div>
                 <label className={labelClass}>Provinsi{requiredIndicator}</label>
            ...
            <div className="card">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                 <div>
                   ...
                   <select value={form.dom_provinsi} onChange={(e) => setForm({ ...form, dom_provinsi: e.target.value, dom_kabupaten: '', dom_kecamatan: '', dom_desa: '' })} className={selectClass}>
                     <option value="">Pilih Provinsi</option>
                     {PROVINSI_OPTIONS.map(p => <option key={p} value={p}>{p}</option>)}
                   </select>
                 </div>
                 <div>
                   ...
                   <select value={form.dom_kabupaten} onChange={(e) => setForm({ ...form, dom_kabupaten: e.target.value, dom_kecamatan: '', dom_desa: '' })} className={selectClass}>
                     <option value="">Pilih Kabupaten/Kota</option>
                     {(KABUPATEN_OPTIONS[form.dom_provinsi] || []).map(k => <option key={k} value={k}>{k}</option>)}
                   </select>
                 </div>
                 <div>
                   ...
                   <select value={form.dom_kecamatan} onChange={handleDomKecamatanChange} className={selectClass}>
                     <option value="">Pilih Kecamatan</option>
                     {getKecamatanOptions(form.dom_provinsi, form.dom_kabupaten).map(k => <option key={k} value={k}>{k}</option>)}
                   </select>
                 </div>
                 <div>
                   ...
                   <select value={form.dom_desa} onChange={(e) => setForm({ ...form, dom_desa: e.target.value })} className={selectClass}>
                     <option value="">Pilih Desa/Kelurahan</option>
                     {getDesaOptions(form.dom_provinsi, form.dom_kabupaten, form.dom_kecamatan).map(d => <option key={d} value={d}>{d}</option>)}
                   </select>
                 </div>
                <div className="sm:col-span-2 lg:col-span-3">
                  <label className={labelClass}>Alamat</label>
                  <textarea name="alamat" value={form.alamat} onChange={(e) => setForm({ ...form, alamat: e.target.value })} className={inputClass} rows={2} placeholder="Alamat domisili" />
                </div>
                <div>
                  <label className={labelClass}>RT</label>
                  <input type="text" name="rt" value={form.rt} onChange={(e) => setForm({ ...form, rt: e.target.value })} className={inputClass} placeholder="RT" />
                </div>
                <div>
                  <label className={labelClass}>RW</label>
                  <input type="text" name="rw" value={form.rw} onChange={(e) => setForm({ ...form, rw: e.target.value })} className={inputClass} placeholder="RW" />
                </div>
                <div>
                  <label className={labelClass}>Kode Pos</label>
                  <input type="text" name="kode_pos" value={form.kode_pos} onChange={(e) => setForm({ ...form, kode_pos: e.target.value })} className={inputClass} placeholder="Kode Pos" />
                </div>
                <div>
                  <label className={labelClass}>No Telp</label>
                  <input type="text" name="no_telp" value={form.no_telp} onChange={(e) => setForm({ ...form, no_telp: e.target.value })} className={inputClass} placeholder="No Telp" />
                </div>
                <div>
                  <label className={labelClass}>Status Kepemilikan</label>
                  <input type="text" name="dom_status_kepemilikan" value={form.dom_status_kepemilikan} onChange={(e) => setForm({ ...form, dom_status_kepemilikan: e.target.value })} className={inputClass} placeholder="Status kepemilikan" />
                </div>
                <div>
                  <label className={labelClass}>Status Tinggal</label>
                  <input type="text" name="dom_status_tinggal" value={form.dom_status_tinggal} onChange={(e) => setForm({ ...form, dom_status_tinggal: e.target.value })} className={inputClass} placeholder="Status tinggal" />
                </div>
                <div className="sm:col-span-2 lg:col-span-3">
                  <label className={labelClass}>Catatan</label>
                  <textarea name="dom_catatan" value={form.dom_catatan} onChange={(e) => setForm({ ...form, dom_catatan: e.target.value })} className={inputClass} rows={2} placeholder="Catatan domisili" />
                </div>
              </div>
            </div>
          )}
