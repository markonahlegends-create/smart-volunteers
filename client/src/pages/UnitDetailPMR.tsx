...
export default function UnitDetailPMR() {
  ...
  const { data: unit, isLoading: unitLoading, error } = useQuery({
    queryKey: ['unit-pmr', id],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('Token tidak ditemukan. Silakan login kembali.');
      }
      const res = await fetch(`/api/units/pmr/${id}`, {
  ...
  const { data: members = [] } = useQuery({
    queryKey: ['members-pmr', id, activeTab],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/units/pmr/${id}/anggota/${encodeURIComponent(activeTab)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) return [];
      return res.json();
  ...
  const deleteMutation = useMutation({
    mutationFn: (memberId: number) => membersApi.deletePMR(memberId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['members-pmr', id] });
    },
  });

  const updateUnitMutation = useMutation({
    mutationFn: (data: any) => unitsApi.updatePMR(Number(id), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['unit-pmr', id] });
  ...
  const uploadSk = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/units/pmr/${id}/upload-sk`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      <div>
        ...
        <div className="text-center py-12">
          ...
          {isAuthError ? (
            ...
            <Link to="/units/pmr/mula" className="btn-primary mt-4 inline-block">Kembali ke Daftar Unit</Link>
          )}
        </div>
      </div>
    );
    ...
    <div>
      ...
      <div className="space-y-4 lg:space-y-6">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <button onClick={() => navigate('/units/pmr/mula')} className="flex items-center gap-1 hover:text-primary-600">
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div className="flex-1">
              ...
              {unit?.tingkat && (
                <p className="text-sm text-gray-600 mt-1">Tingkat: {unit.tingkat}</p>
              )}
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => navigate(`/members/pmr/add?unitId=${id}`)} className="btn-primary flex items-center gap-2 text-sm">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{unit?.tingkat}</p>
              </div>
            </div>
          </motion.div>
        </div>
                  ...
                  <tr key={member.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => navigate(`/members/pmr/${member.id}`)}
                        ...
                        <button
                          onClick={() => navigate(`/members/pmr/add/${member.id}?unitId=${id}`)}
                          className="p-1 text-primary-600 hover:bg-primary-50 rounded"
                          title="Edit Anggota"
                        >
          ...
          <form onSubmit={handleUnitSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select name="tingkat" value={unitForm.tingkat} onChange={(e) => setUnitForm({ ...unitForm, tingkat: e.target.value })} className="input-field">
            ...
            <div>
              ...
              <div
                ...
                onClick={() => document.getElementById('sk-upload-modal-pmr')?.click()}
              >
