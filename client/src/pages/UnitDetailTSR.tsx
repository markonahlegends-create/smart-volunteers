...
export default function UnitDetailTSR() {
  ...
  const { data: unit, isLoading: unitLoading, error } = useQuery({
    queryKey: ['unit-tsr', id],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('Token tidak ditemukan. Silakan login kembali.');
      }
      const res = await fetch(`/api/units/tsr/${id}`, {
  ...
  const { data: members = [] } = useQuery({
    queryKey: ['members-tsr', id, activeTab],
    queryFn: async () => {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/units/tsr/${id}/anggota/${encodeURIComponent(activeTab)}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) return [];
      return res.json();
  ...
  const deleteMutation = useMutation({
    mutationFn: (memberId: number) => membersApi.deleteTSR(memberId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['members-tsr', id] });
    },
  });

  const updateUnitMutation = useMutation({
    mutationFn: (data: any) => unitsApi.updateTSR(Number(id), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['unit-tsr', id] });
  ...
  const uploadSk = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/units/tsr/${id}/upload-sk`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      <div>
        ...
        <div className="text-center py-12">
          ...
          {isAuthError ? (
            ...
            <Link to="/units/tsr" className="btn-primary mt-4 inline-block">Kembali ke Daftar Unit</Link>
          )}
        </div>
      </div>
    );
    ...
    <div>
      ...
      <div className="space-y-4 lg:space-y-6">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <button onClick={() => navigate('/units/tsr')} className="flex items-center gap-1 hover:text-primary-600">
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => navigate(`/members/tsr/add?unitId=${id}`)} className="btn-primary flex items-center gap-2 text-sm">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{unit?.jenis || 'TSR'}</p>
              </div>
            </div>
          </motion.div>
        </div>
                  ...
                  <tr key={member.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => navigate(`/members/tsr/${member.id}`)}
                        ...
                        <button
                          onClick={() => navigate(`/members/tsr/add/${member.id}?unitId=${id}`)}
                          className="p-1 text-primary-600 hover:bg-primary-50 rounded"
                          title="Edit Anggota"
                        >
        ...
        >
          <form onSubmit={handleUnitSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="jenis" value={unitForm.jenis} onChange={(e) => setUnitForm({ ...unitForm, jenis: e.target.value })} className="input-field" required />
            </div>
            <div>
            ...
            <div>
              ...
              <div
                ...
                onClick={() => document.getElementById('sk-upload-modal-tsr')?.click()}
              >
