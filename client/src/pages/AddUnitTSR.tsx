...
interface AddUnitTSRProps {
  backUrl: string;
}

export default function AddUnitTSR({ backUrl }: AddUnitTSRProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await unitsApi.createTSR(data);
      return res;
    },
    onSuccess: async (data) => {
      if (selectedSkFile) {
        ...
        await fetch(`/api/tsr/${data.id}/upload-sk`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
        });
      }
      queryClient.invalidateQueries({ queryKey: ['units', 'tsr'] });
      navigate(backUrl);
    },
  });
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div
                ...
                onClick={() => document.getElementById('sk-upload-add-tsr')?.click()}
              >
