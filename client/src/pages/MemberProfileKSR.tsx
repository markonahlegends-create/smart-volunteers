...
export default function MemberProfileKSR() {
  ...
  const { data: member, isLoading } = useQuery({
    queryKey: ['member-ksr', id],
    queryFn: async () => {
      const res = await fetch(`/api/members/ksr/${id}`, {
  ...
  const handleEdit = () => {
    navigate(`/members/ksr/add/${id}`);
  };

  if (isLoading) {
    return (
      <div>
        <Header title="Profil Anggota KSR" />
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">{member.jenis || 'KSR'}</span>
