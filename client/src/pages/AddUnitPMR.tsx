...
const TINGKAT_OPTIONS = ['MULA', 'MADYA', 'WIRA'];
const PROVINSI_OPTIONS = ['BANTEN'];
const KABUPATEN_OPTIONS = ['KOTA CILEGON', 'LEBAK', 'PANDEGLANG', 'SERANG', 'KABUPATEN SERANG'];

interface AddUnitPMRProps {
  tingkat: string;
  backUrl: string;
}

export default function AddUnitPMR({ tingkat, backUrl }: AddUnitPMRProps) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [form, setForm] = useState({
    kode_unit: '',
    nama_unit: '',
    tingkat: tingkat || 'MULA',
    email: '',
    no_telpon: '',
    status: 'Aktif',
    provinsi: '',
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await unitsApi.createPMR(data);
      return res;
    },
    onSuccess: async (data) => {
      if (selectedSkFile) {
        ...
        await fetch(`/api/pmr/${data.id}/upload-sk`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
        });
      }
      queryClient.invalidateQueries({ queryKey: ['units', 'pmr'] });
      navigate(backUrl);
    },
  });
  ...
  return (
    <div>
      <Header title="Tambah Unit PMR" />
      <div className="space-y-4 lg:space-y-6">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate(backUrl)} className="flex items-center gap-1 text-sm text-gray-600 hover:text-primary-600">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              ...
              <select name="tingkat" value={form.tingkat} onChange={(e) => setForm({ ...form, tingkat: e.target.value })} className="input-field">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div
                ...
                onClick={() => document.getElementById('sk-upload-add-pmr')?.click()}
              >
          ...
          <div className="lg:col-span-2 flex gap-3 pt-4">
            ...
            <button type="button" onClick={() => navigate(backUrl)} className="btn-secondary">
              Batal
            </button>
          </div>
        </form>
